...
| **840 → 960 → 900 → 830** | The window width at which `NSToolbar` stops pushing items into the » overflow, swept in 10 pt steps against `visibleItems`: before the «Из»/«В»/«Тон» labels existed, with each label as its own toolbar item, with each label paired into its control's item, and with the longest language name shortened. The window opens at 900, so the middle figure is a primary action hidden at the default size. `.controlSize(.regular)` against the toolbar's own default was measured at **10 pt** and rejected as not worth leaving the platform metrics for. Re-measure with `Scripts/toolbar-fit.swift`. | `MainWindowView.swift`'s toolbar, `Language.russianName` |
