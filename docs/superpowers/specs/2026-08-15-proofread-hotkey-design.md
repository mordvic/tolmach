...
| The system refuses the правка combination | `apply()` restores the previous one, as for перевод, and the failure is logged at `.error` — **not** `.fault`. `fault` is justified for перевод by «the app has no shortcut and no way into the panel»; that door stays open here, and copying the level would make the log lie about severity |
