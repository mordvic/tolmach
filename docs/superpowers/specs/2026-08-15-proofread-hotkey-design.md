...
| One shortcut moved onto the combination the other holds | Both registrations are brought in line in **two passes** — release everything that must not be registered, then register what must. Measured: with one pass in `allCases` order, changing перевод to правка's combination left перевод on its *previous* one, because Carbon refused while правка still held it and `apply` restored. The user's choice was discarded silently |
