...
**Пустой ответ модели неотличим от перевода в пустоту.** `TranslationOutcome` не несёт признака,
позволяющего различить «модель ничего не сказала» и «перевод оказался пустым». Раздел 8 обещает
сообщение об ошибке с кнопкой повтора; реализовать его без изменения ядра нельзя.
