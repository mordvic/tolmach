...
> **Статус: все три волны выполнены.** `wave1/observability-and-swift6` (PR #9),
> `wave2/mac-idioms` (PR #10) и `wave3/settings-a11y-and-ci` — стек, ни одна не слита.
>
> Закрыты **A1, A2, A3, A4, A5, A6, A7/M1, M2, M5, M11** и **U1/M3, U2, U3/M4, U4, U5/M6, U8**,
> плюс drag & drop из U7. **U6 опровергнут** (ниже), Services из U7 отложены с причиной.
> Проверено: холодная `swift build --build-tests` — 0 ошибок, 0 предупреждений; `swift test` —
> 364 теста зелены в 3 прогонах из 3; `-strict-concurrency=complete` — 0 предупреждений;
> сквозной прогон `translate-cli` против живой Ollama; локализация проверена на **собранном
> бандле**; подпись цела.
>
> ### Три места, где отчёт оказался неправ, и это выяснилось измерением
>
> 1. **U6 опровергнут целиком.** Утверждалось, что жёсткая рамка `.frame(width:height:)` в
>    настройках обрезает содержимое, и рекомендовалось заменить её на `minWidth/minHeight` плюс
>    `ScrollView`. Контрольный опыт: `.formStyle(.grouped)` **сам ставит `NSScrollView`** — при
>    любом объёме содержимого, — тогда как `VStack` и `Form` без стиля не ставят ни одного.
>    Настройки уже прокручиваются. Рекомендованная правка вернула бы прыжки окна между
>    вкладками, ради которых рамка и существует, и не дала бы ничего. `SettingsPane` не тронут.
> 2. **U3 частично неправ.** `CommandGroup(replacing: .sidebar) { }` не убирает пустое меню
>    «Вид» — группа опустошается, но заголовок остаётся, то есть становится хуже. Понадобился
>    `pruneEmptyMenus()` на AppKit.
> 3. **U4 нельзя проверить тем инструментом, которым мерил и сам проект.** `OPEN-ITEMS` §2
>    ссылался на «`entire contents` панели пусто через System Events». Проверено: обход дерева
>    даёт `AXWindow → AXGroup` без метки и без детей **одинаково с новыми модификаторами и
>    без них**. SwiftUI не материализует дерево, пока не подключён AT-клиент. Наблюдение не
>    было свидетельством о панели; запись в `OPEN-ITEMS` исправлена.
>
> ### Находка, которой в отчёте не было вовсе
>
> **Глоссарий по умолчанию показывал не ту колонку — U12.** Найдено не чтением, а отрисовкой:
> панель настроек, снятая с четырьмя заполненными терминами, показала пустое поле «перевод» у
> каждого. Причина — `editingLanguage` брал `settings.workingLanguage` (английский), тогда как
> `targetLanguage(forDetected:)` отправляет всё, что не на языке пользователя, **в** его язык,
> то есть в русский. На установке по умолчанию глоссарий наполняется `translations["ru"]`, а
> панель показывала колонку `en`. Объяснить это было неоткуда: пикер языка `.labelsHidden()` и
> подписан только тултипом.
>
> Исправлено `GlossaryColumn` — язык выводится из содержимого глоссария, а не из настройки, так
> что обе стороны перевода работают. Ветка `fix/glossary-language-default`.
>
> **Отложено с причиной:** Services из U7. Это не техдолг, а новая поверхность продукта, и она
> требует решения, которого я принять не могу: что «Перевести Толмачом» делает с результатом —
> возвращает замену выделенного текста в чужой документ или показывает панель. Первое меняет
> чужой документ, второе дублирует хоткей. Плюс проверить это отсюда нечем.
>
