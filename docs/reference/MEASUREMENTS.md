...
**This file is an index, not the authority** — same standing as `docs/reference/PLATFORM-TRAPS.md`. Each
figure is owned by a code comment or by `docs/reference/BASELINE.md`; those are kept true by sitting next
...
- **Perishable** — what the engine scores today. Re-measured by `swift run acceptance` on every
  run, machine-dependent, and recorded in `docs/reference/BASELINE.md` rather than here. **No perishable
...
| **380 × 120** → **380 × 260** | The panel before and after `sizingOptions = []`. The first is what `NSHostingView` compressed it to regardless of content. **Quarantined:** taken on a `.titled` panel against a fixed size, and neither condition exists since the UI redesign. The mechanism stands; the figures cannot be reproduced without the bundle on a screen. | `TranslationPanel.swift`, `docs/reference/OPEN-ITEMS.md` |
...
| **242 pt** against **272** | The panel's степень/стиль row at the width floor, with the longest label in each picker («ошибки и стиль», «профессиональный»), `.menu` at `.controlSize(.mini)`: it wants 242 × 16, and the 300 pt floor leaves 272 after `PanelView`'s 14 pt padding a side. So the row fits without widening the panel, and the fallback shapes the design listed are not needed — the stacked variant wants 122 × 36 and is kept in the probe as the thing to reach for if a label ever grows. **30 pt of headroom is not much**: one longer стиль name spends it. Re-measure with `Scripts/panel-proofread-row.swift`, and treat its absolutes as approximate the way `Scripts/toolbar-fit.swift`'s header says to — it builds the row alone, not inside the real panel. | `PanelView.proofreadingControls` |
| **status 0**, and **24 entries with no `64`** | The правка shortcut's factory combination, ⌥⌘R — key code 15, Cocoa modifiers 1572864. `RegisterEventHotKey` accepts it, and no *customised* system shortcut in `com.apple.symbolichotkeys` holds it. **The second half is weaker than it reads, and that was measured rather than assumed**: the plist holds deviations from the factory set, not the set. Spotlight's ⌘Space — entry `64`, the most famous system shortcut there is — is absent from all 24 entries on this machine, while `60` and `61`, the input-source shortcuts this user had changed, are present. So a clean answer rules out a *customised* collision and nothing more; the factory set is checked against Apple's documentation by hand, and the only real proof is a press on a built bundle, which `docs/reference/OPEN-ITEMS.md` carries as a manual check. Re-measure with `Scripts/hotkey-availability.swift`. | `HotkeyCombo.proofreadDefault` |
...
## Durable — change marks (measured 2026-09-04; spec #81, PR 5)

All on this machine, Ollama 0.33.2 where a model is involved. Item numbers are the spec's
measurement protocol.

| Fact | Where |
|---|---|
| **Item 1 — `densityThreshold` stays 0.5.** `Scripts/change-density.sh docs/proofreading-gate`: 12 texts × 3 степени × 3 runs on `translategemma:12b` and `:27b`, 51 block pairs per степень per model. Changed-token ratio (`changedTokens / (source + result)`), p50 / p90 / max — 12b: «только ошибки» 0.06 / 0.12 / 0.15, «ошибки и стиль» 0.14 / 0.38 / 0.46, «переписать» 0.24 / 0.50 / 1.00; 27b: 0.08 / 0.15 / 0.15, 0.16 / 0.44 / 1.00, 0.29 / 0.45 / 1.00. The similarity pre-check (`1 − Dice`) reads the same to two decimals except where a block was rewritten whole. **Every** «только ошибки» block on both models is below 0.30; the share of «переписать» blocks above the threshold is 41 % / 47 % at 0.30, 12 % / 6 % at 0.50 — the spec's «≥ 90 % above» half is unreachable because the models rewrite lightly, and its fallback («record both, keep 0.5») applies. What 0.5 collapses is exactly the blocks a model rewrote whole (ratio 1.00: 6 % of rewrites on 12b, and — worth knowing — 10 % of «ошибки и стиль» blocks on 27b) | `TextDiff.changes(densityThreshold:)`, `Scripts/change-density.sh` |
| **Item 2 — `inspectionLimit` 60 000 and `blockTokenLimit` 4 000 stay, with a stated deviation.** `TEXT_DIFF_COST=1 swift test --filter textDiffCost`, median of 5: 256 KB prose (19 825 words) with 1 % of words changed **133 ms**; every other word changed **187 ms**; one 4 000-token paragraph reordered **24 ms**; 200 paragraphs each rewritten **56 ms**. The spec wanted the worst case under 50 ms; the two 256 KB cases are not, and the limit was kept because the cost lands off the main actor at the end of a minutes-long run, and a limit that met 50 ms would take the marks away from documents over ~60 KB | `TextDiff.changes(inspectionLimit:blockTokenLimit:)`, `Tests/TranslationCoreTests/TextDiffCostProbe.swift` |
| **Item 3 — the перевод pane's header does not fit 280 pt with any picker shape.** `Scripts/pane-header-fit.swift`, `fittingSize` of the header with «Правка», the picker, «Ещё вариант», «Скопировать»: three segments 495 pt, today's two 397 pt, a `.menu` 352 pt — and the 280 pt floor fit none of them. **Fixed the same day**: the floor is 360 (the first round number past the menu form) and the picker sits in a `ViewThatFits` that falls back to the `.menu` form, so nothing clips and segments appear wherever they fit | `TranslationPane` (`ViewThatFits`, `minWidth: 360`), `PaneViewChoice` |
| **Item 4 — the panel's степень/стиль/вид row wants 331 × 16 pt inline** (241 without «Вид»; stacked 122 × 56) against the 272 pt the 300 pt floor leaves — and the real правка panel asks 367 / 440 / 442 / 481 pt in its four states, so the row is inline and the «floor» is not a width the panel reaches | `PanelView.proofreadingControls`, `Scripts/panel-proofread-row.swift` |
| **Item 5 — `PanelSizer.minHeight` 132 stays; `dragMinHeight` 164 → 179** («finished правка + окно занято», the tallest pinned block the marks create). Every перевод figure in both tables reads +6 pt on the old table, uniformly, in untouched states; three older states measure 194 / 204 / 228 and were already past 164 — recorded, not chased, because a 228 floor would jump every перевод panel on first drag | `PanelSizer` (both tables), `TranslationPanel.contentMinSize` |
