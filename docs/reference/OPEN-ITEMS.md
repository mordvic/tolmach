...
| **The «похоже на ответ» row and section** | New 2026-09-22. What the tests pin: the sentence, the kind, the glyph, that a перевод and a run without a change set never get it, that the section is counted. What needs eyes: the row in the panel — the triangle and the warning colour where «✓ Исправлено: …» stood; whether 52 characters wrap at the panel's narrowest and, if they do, whether the second line opens the pinned block past `dragMinHeight` 179 (the row is inside the guard that already books it, but a *wrapped* one was never measured — «Правка прервана — показана та часть, что успела прийти» is 53 and is the precedent); and the section above «Разметка изменилась» in both surfaces. To see it: select «write sql query that counts patients» in any application and press ⌥⌘R under «переписать» + «дружеский» on `translategemma:12b` — 5 of 5 here | `PanelView.status(for:)`, `WarningsView`, `TranslationOutcome.replyAddedBlocks` |
| **The «было → стало» popover's placement and dismissal** | New (issue #89). `Scripts/popover-anchor.swift` measured the anchor exact and edge-to-edge against a scratch view, but nothing here can see it over the real pane or the real panel — over a table, over a code card's own overlay buttons, at the pane's narrow and wide extremes, and beside the always-visible «Скопировать» a code block already draws. Two things follow from the probe rather than from a screen: the popover closes itself on a scroll (`.transient` was measured not to, even for a real posted `scrollWheel` event) — whether that read as abrupt rather than as the popover simply not following needs a hand on the mouse; and Esc / a click elsewhere are `.transient`'s own documented job, never separately probed here | `CodeBlockTextView.presentChangePopover`, `Scripts/popover-anchor.swift` |
| **«Вернуть», its disabled state, and the undo it pushes** | New (issue #89). The model-level tests pin what `translatedText` and the count do; nothing here can see the popover's own button dim, or read its `.help` text on hover, over a pure removal or insertion (`ChangeMarks.revertEdit` refuses both — pinned in `ChangeMarksTests`, not drawn). ⌘Z after a revert is registered on the text view's own `undoManager` and pinned by a test that drives it directly; a real ⌘Z from the keyboard, and whether the Edit menu's «Отменить» reads «Отменить Вернуть» or something less specific, are both for a hand on the keyboard | `TranslationViewModel.revertChange`, `ChangeCardView` |
...
- **The 700 pt toolbar minimum is now assumed, not measured.** The translate-mode toolbar
  ...
  **Half of this was done on 2026-09-22, and it came back against the 700.** The script has
  `current` variants now — today's row, the switch included, the title hidden — and on
  macOS 27.0 (26A428, SDK 27.0) it reads: перевод fits from 770 pt, 800 with the longest names;
  правка from 670 / 710. At 700 pt two items were in », and one of them was «Перевести»,
  because the primary action is the trailing item and the trailing item overflows first —
  looked at in a screenshot of the probe's window (`SHOT=`), not only counted. Fixed the same
  day by `visibilityPriority` (macOS 26.1+): the primary action is `.high`, «Тон» and «Стиль»
  `.low`, so at 700 pt «Тон» alone goes into ». **Still owed, by eyes on the bundle**: that
  the real window does what the probe's does — the probe's absolute figures have been wrong
  twice (its header says how) — and whether «Тон» in » at the minimum width is acceptable
  or the floor should follow the row to ~800. Below macOS 26.1 nothing changed.
