...
| «Открыть в окне» with a **finished** translation already in the window | The hand-off moves `outcome`, `resolvedTarget` and `state` together; the failure it fixes was the window showing the previous run's elapsed time and warnings under the new text. The window it hands to has since been rebuilt. Since 2026-08-15 it also moves the five per-run overrides, so the toolbar's «Из», «В», «Тон», «Степень» and «Стиль» should all describe the adopted run after the hand-off — including *clearing* a selection the window had made, which is the half worth looking at | `TranslationViewModel.adopt(from:)` |
