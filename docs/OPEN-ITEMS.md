...
| **How the paired toolbar reads**, and the window at its new 840 pt minimum | The arithmetic is settled and the taste is not. Measured on the bundle at 840, 860, 900 and 1000 pt: 5 of 5 toolbar items visible, no overflow, no title field. What a person has to say is whether «Из» at 6 pt from its pop-up reads as belonging to it, whether the row is still crowded at 840 even though it fits, and whether a window that no longer narrows below 840 is a loss worth the primary action always being on screen | `MainWindowView.swift` |
| «китайский» in all five pickers, and in «будет переводиться на …» | The one user-facing string this work changed. Nothing here can judge whether losing «(упрощённый)» costs a reader something the prompt's own «Chinese (Simplified)» does not cover — the app offers one Chinese, so it distinguished nothing on offer, but a person who cares about script may still want it said | `Language.russianName` in `RussianCopy.swift` |
