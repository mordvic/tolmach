...
@Test func everyRedirectIsRefused() async {
    let policy = RedirectPolicy()
    let session = URLSession(configuration: .ephemeral)
    let task = session.dataTask(with: URLRequest(url: OllamaClient.defaultBaseURL))
    defer { task.cancel(); session.invalidateAndCancel() }

    for status in [301, 302, 303, 307, 308] {
        let response = HTTPURLResponse(url: OllamaClient.defaultBaseURL, statusCode: status,
...
@Test func aRedirectToLoopbackIsRefusedAsWell() async {
    let policy = RedirectPolicy()
    let session = URLSession(configuration: .ephemeral)
    let task = session.dataTask(with: URLRequest(url: OllamaClient.defaultBaseURL))
    defer { task.cancel(); session.invalidateAndCancel() }

    let response = HTTPURLResponse(url: OllamaClient.defaultBaseURL, statusCode: 307,
                                   httpVersion: "HTTP/1.1", headerFields: [:])!
    let followed: URLRequest? = await withCheckedContinuation { continuation in
        policy.urlSession(session, task: task, willPerformHTTPRedirection: response,
                          newRequest: URLRequest(url: OllamaClient.baseURL(port: 11435))) {
            continuation.resume(returning: $0)
        }
    }
    #expect(followed == nil)
