...
@Test func parsesContentToken() throws {
    let events = try OllamaStreamParser.parse(line: #"{"message":{"role":"assistant","content":"OK"},"done":false}"#)
    #expect(events.count == 1)
    guard case .token(let text) = events.first else { Issue.record("expected token"); return }
    #expect(text == "OK")
}

@Test func discardsThinkingAndEmptyContent() throws {
    let events = try OllamaStreamParser.parse(line: #"{"message":{"role":"assistant","thinking":"let me think","content":""},"done":false}"#)
    #expect(events.isEmpty)
}

@Test func parsesDoneWithNanosecondToMillisecondConversion() throws {
    ...
    let events = try OllamaStreamParser.parse(line: line)
    #expect(events.count == 1)
...
@Test func returnsEmptyForBlankOrGarbageLine() throws {
    #expect(try OllamaStreamParser.parse(line: "").isEmpty)
    #expect(try OllamaStreamParser.parse(line: "not json").isEmpty)
}

@Test func combinedContentAndDoneFrameReturnsBothEventsTokenFirst() throws {
